...
Phase III is complete. The fix is implemented, tested, pushed, and open as a **pull request** (marked ready for review in Phase IV) — [modernuo/ModernUO#2498](https://github.com/modernuo/ModernUO/pull/2498).
...
- **PR:** [modernuo/ModernUO#2498 — feat(regions): create regions for all vendor shops (#1052)](https://github.com/modernuo/ModernUO/pull/2498)
- **Files changed:**
...
Phase IV turns the Phase III draft into a professional, review-ready submission and records it here.

### Pull Request

- **PR Link:** [modernuo/ModernUO#2498 — feat(regions): create regions for all vendor shops (#1052)](https://github.com/modernuo/ModernUO/pull/2498)
- **Base ← Head:** `modernuo/ModernUO:main` ← `Jynx-hub/ModernUO:fix-issue-1052`
- **Closes:** [#1052](https://github.com/modernuo/ModernUO/issues/1052)

### What I Contributed (summary)

Added **394 shop-specific `NoHousingRegion` regions** to `Distribution/Data/regions.json`, giving every vendor shop across **23 towns** on **all 6 facets** (Trammel, Felucca, Ilshenar, Malas, Tokuno, Ter Mur) its own named region, plus a CI-safe data-validation test suite. The change is **data-only** — it reuses the existing region engine (parent/child resolution) and the established New Haven / Haven shop-region pattern, so no engine or content code was modified. PR diff: **2 files** — `Distribution/Data/regions.json` (+3643 / -145) and `Projects/Server.Tests/Tests/Regions/VendorShopRegionTests.cs` (+242).

### Pre-Submission Checks

- Merged latest upstream `main` into the branch — PR is **mergeable**, no conflicts.
- Removed `CONTRIBUTION_SETUP.md` from the PR branch so the PR contains only the fix and its tests. This record lives on my fork's `main` instead of the upstream PR.
- Reviewed the full `git diff` against upstream `main`: only the two intended files remain.
- Tests (after merging upstream `main`): `dotnet test --filter VendorShopRegion` → **28 passed / 0 failed**; full `Server.Tests` → **728 passed / 0 failed / 17 skipped** (skips need client tile data; unrelated).

### Maintainer Feedback

No review yet as of submission — **awaiting maintainer feedback**. I left a comment on the PR tagging the primary maintainer ([@kamronbatman](https://github.com/kamronbatman), who merges nearly all recent PRs) requesting a review. As feedback arrives I will document each round of comments and how I addressed them here, and update the status below.
